...
return new class extends Migration {
    ...
    {
        // 1. Equipment Categories (Free Weights, Cardio, Machines, Racks)
        ...
        Schema::create('equipment', function (Blueprint $table) {
            $table->id();
            $table->string('equipment_code', 30)->unique(); // EQP-001
            $table->foreignId('category_id')->constrained('equipment_categories')->restrictOnDelete();
            $table->string('name', 150);
            $table->string('brand', 100)->nullable();
            $table->unsignedInteger('quantity')->default(1);
            $table->string('location_in_gym', 100); // Free Weights Area, Cardio Deck
        ...
        // 3. Equipment Maintenances (Service records, facility locations, and repairs)
        Schema::create('equipment_maintenances', function (Blueprint $table) {
            $table->id();
            $table->string('maintenance_code', 30)->unique(); // MNT-001
